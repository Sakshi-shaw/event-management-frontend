...
const BASE_URL = 'http://localhost/ci4/backend'; // Replace with your actual API base URL
...
const Global = {
    ...
    isAuthenticated: () => {
      const userId = Global.getUserId();
      const userRole = Global.getUserRole();
      const userRoleId = Global.getUserRoleId();
      return userId !== 0 && userRole !== '' && userRoleId !== 0;
    },
    ...
    logout: async () => {
        ...
        window.location.reload(); // Reload the page after logout to update the header
  ...
  //   if (typeof id === 'number') {
  //     Global.userId = id;
  //     console.log('Global.userId'+Number(Global.userId))
  //   } else {
  //     console.error('Invalid userId type. It must be a number.');
  //   }
  // },
  ...
  //   if (typeof role === 'string') {
  //     Global.userRole = role;
  //     console.log('Global.userRole'+String(Global.userRole))
  //   } else {
  //     console.error('Invalid userId type. It must be a number.');
  //   }
  // },
  ...
  // isAuthenticated: () => {
  //   // Example implementation: Check if a token exists in localStorage
  ...
  login: async (userData) => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        const { logid, id, data } = response.data; // Destructure response data

      // Store id in the global variable
        Global.setUserId(Number(id));
        Global.setUserRole(String(data.role));
        Global.setUserRoleId(Number(id));

      // Optionally store id and other relevant data in localStorage for persistence
      // localStorage.setItem("userId", id);
      // localStorage.setItem("id", response.data.token); // Assuming token is part of the response
      ...
      console.log("user role:", Global.userRole);
      console.log("Logged in user RoleId:", data.role_id);

      console.log("user roleId:", Global.userRoleId);


        //Global.userId = id;
       //console.log(Global.userId);
console.log(Number(response.id))
//console.log("User ID: ",Number(data.id)); 
        //console.log("ji"+JSON.stringify(data[id]))
        return response.data; // Return response data if registration is successful
      } else {
        // Handle unsuccessful registration
        throw new Error(response.data.message || 'Error login user');
      }
    } catch (error) {
...
registerUser: async (userData) => {
    ...
    createEvent: async (eventData) => {
      //const event={'teacher_id':String(Global.userId), eventData};
    //  eventData:{'teacher_id':String(Global.userId),eventData}
    console.log("from create"+Global.userId);
    eventData = {
    ...
    updateStudent: async (studentData) => {
      try {
        ...
        if (response.status === 200 || response.data.succes===true) {
  ...
  fetchEventLevel: async () => {
    try {
      const response = await axios.get(`${BASE_URL}/eventlevels`); // API endpoint for students
      console.log("eventlevels: "+ JSON.stringify(response.data));
      if (response.status === 200) {
        return response.data; // Return student data if the request is successful
      } else {
        throw new Error(response.data.message || 'Failed to fetch eventlevels.');
      }
    } catch (error) {
      console.error('Error fetching eventlevels:', error);
      throw new Error(error.response?.data?.message || 'Failed to fetch eventlevels.');
    }
  } ,
...
const api = axios.create({
  ...
  const secretKey = "EventManagement"; // Must be kept secure!
